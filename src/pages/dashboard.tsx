...
import { useRoqPlatformUserProfiles, useSalesCount, useProductCount, useOutletCount } from 'lib/roq';

import { SalesListPage as ShowCaseList } from 'pages/sales';

function DashboardPage() {
  const { session } = useSession();

  const salesStat = useSalesCount();

  const productStat = useProductCount();

  const outletStat = useOutletCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Sales',
      result: salesStat?.data,
