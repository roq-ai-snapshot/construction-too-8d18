...
import { useRoqClient, useOutletFindManyWithCount } from 'lib/roq';
...
export function OutletListPage(props: OutletListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['company', 'product.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'address.contains', 'city.contains', 'state.contains', 'zip.contains'],
          ...params.filters,
        },
        'outlet',
      ),
  ...
  const { data, error, isLoading, mutate } = useOutletFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.outlet.delete({ where: { id } });
      await mutate();
    } catch (error) {
