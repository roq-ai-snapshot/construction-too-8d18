...
import { useRoqClient, useOutletFindFirst } from 'lib/roq';
...
function OutletEditPage() {
  ...
  const { data, error, isLoading, mutate } = useOutletFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: OutletInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.outlet.update({
        data: values as RoqTypes.outlet,
        where: {
          id,
        },
      });
